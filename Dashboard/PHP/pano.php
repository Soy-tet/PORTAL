<?php 
   ...
   if(!$connection_team){
    ...
    $all_data_pano = $connection_team->query("SELECT * FROM `pano`;");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="body">
   <div class="global">
     <?php include("../../component/card/card.php"); ?>
        ...
        <?php
        $i=0;
        while($nav_pano = mysqli_fetch_assoc($all_data_pano)){
            ?>  
                <tr class="tr">
                    <td><?php echo $nav_pano["id"]; ?></td>
                    <td><?php echo $nav_pano["name"]; ?></td>
                    <td><img class="image-table-team" src="<?php echo $nav_pano["image"]; ?>" alt=""></td>
                    <td><?php echo $nav_pano["time"]; ?></td>
                    <td class="remove"><a href="/PORTAL/PORTAL/Dashboard/SQL/INSERT/update/update_pano.php?userid=<?php echo $nav_pano["id"]; ?>" ><i class="fa-sharp fa-solid fa-pen-to-square"></i></a></td>
                </tr>
            <?php
             $i++;
                }
